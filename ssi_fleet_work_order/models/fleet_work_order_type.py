...
class FleetWorkOrderType(models.Model):
    ...
    @api.depends("route_template_selection_method", "route_template_ids", "route_template_domain")
    def _compute_allowed_route_template_ids(self):
        for record in self:
            ...
            if record.route_template_selection_method == 'manual':
                ...
                result = self.env['fleet_work_order_route_template'].search(criteria).ids
